function createLoginGateModel(pageTitle) {
  const pageConfig = {
    ...
    '发送提醒': {
      icon: '发',
      highlights: ['恢复你的发送身份', '只展示属于你的在线设备', '提交提醒并追踪回执']
    },
    '发送记录': {
      icon: '记',
      highlights: ['读取你的历史提醒记录', '查看设备展示回执', '按账号隔离提醒数据']
    },
    '设备绑定': {
      icon: '绑',
  ...
  const current = pageConfig[pageTitle] || {
    icon: '我',
    highlights: ['恢复你的账号身份', '继续管理头像和昵称', '同步设备和提醒数据']
  }
  ...
  return {
    ...
    description: `完成微信授权登录后，即可继续访问${pageTitle}，并恢复你当前账号下的设备、提醒与记录。`,
