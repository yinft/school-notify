...
public class DeviceApiClientTests
{
    ...
    {
        var handler = new RecordingHandler("{\"device_id\":\"device-001\",\"device_name\":\"值班室电脑\",\"client_version\":\"0.1.0\",\"status\":\"online\",\"last_seen_at\":\"2026-04-21T08:00:00Z\"}");
        ...
        await apiClient.RegisterDeviceAsync(new DeviceRegistrationRequest("device-001", "值班室电脑", "0.1.0"));

        Assert.Equal(HttpMethod.Post, handler.LastRequest!.Method);
        Assert.Equal("http://127.0.0.1:8000/api/devices/register", handler.LastRequest.RequestUri!.ToString());
        Assert.Contains("device-001", handler.LastRequestBody);
    }
    ...
    public async Task RequestBindingCodeAsync_PostsDeviceIdAndReturnsCode()
    {
        ...
        var response = await apiClient.RequestBindingCodeAsync("device-001");
    ...
    public async Task SendHeartbeatAsync_PostsToHeartbeatEndpoint()
    {
        ...
        await apiClient.SendHeartbeatAsync("device-001");
